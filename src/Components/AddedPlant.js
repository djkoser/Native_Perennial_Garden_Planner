  ...
  let list
   ...
   list = myPlantsList.map(el=> {
    if (el.bloom_time===bloomTime) {
      ...
      <span>Common Name:{el.common_name}, Botanical Name:{el.botanical_name}, Moisture:{el.moisture}, Sun:{el.sun}, Height:{el.height}, Bloom Time:{el.bloom_time}, Notes: {editToggle.toggle && editToggle.id ===el.id.toString() ? (<textarea onChange={ev=> handleChange(ev.target.value)} value={notesInput}></textarea>) : el.project_notes}
      </span>
      <br/>
      </li>
      )} else {return null}
