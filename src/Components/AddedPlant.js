  ...
  let list
   ...
   list = myPlantsList.map(el=> {
    if (el.bloom_time===bloomTime) {
      ...
      <span>Common Name:<strong>{el.common_name}</strong>, Botanical Name:<strong><em>{el.botanical_name}</em></strong>, Moisture:{el.moisture}, Sun:{el.sun}, Height:{el.height}, Bloom Time:{el.bloom_time} <button className={el.id} onClick={ev=>toggleEdit(ev.target.className)}>Add Notes</button>{el.project_notes ? (<span>Notes: </span>) : null}{editToggle.toggle && editToggle.id ===el.id.toString() ? (<textarea onChange={ev=> handleChange(ev.target.value)} value={notesInput}></textarea>) : el.project_notes} 
      </span>
      <br/>
      <br/>
      </li>
